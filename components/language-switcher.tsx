...
export function LanguageSwitcher({ lang, label }: LanguageSwitcherProps) {
  ...
  return (
    <nav
      aria-label={label}
      className="flex shrink-0 items-center gap-1 lg:gap-1.5"
    >
      {locales.map((locale) => {
        ...
        return (
          <Link
            ...
            className={
              active
                ? "block size-6 overflow-hidden rounded-sm border-2 border-vermilion transition duration-300 sm:size-8 lg:size-10"
                : "block size-6 overflow-hidden rounded-sm border-2 border-[var(--rule)] transition duration-300 hover:scale-105 hover:border-ink/40 sm:size-8 lg:size-10"
            }
          >
            <Image
              src={shields[locale]}
